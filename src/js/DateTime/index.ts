...
class DateTime extends Control<DateTimeProps> {
  ...
  private _registerTimeTextInputEvents() {
    this._timeTextInput.onclick = () => {
      if (
        ...
        this._timeTextInput.selectionStart &&
        this._timeTextInput.selectionStart >= 2 &&
        this._timeTextInput.selectionStart <= 5
      ) {
    ...
    this._timeTextInput.onblur = e => {
      ...
      if (relatedTarget && this._timePicker.getElement().contains(relatedTarget as Node)) {
        e.preventDefault();
        return;
      }
      this._timePicker.hide();
  ...
  private _setTextInputValueToPreviousValidValue() {
    if (this._timeTextInput.dataset.previousValidTime && this._timeTextInput.dataset.previousValidTime !== this._timeTextInput.value) {
      let previousSelectionStart = 0;
      let previousSelectionEnd = 2;
      if (
        this._timeTextInput.selectionStart &&
        this._timeTextInput.selectionStart &&
        this._timeTextInput.selectionStart >= 3 &&
        this._timeTextInput.selectionStart <= 5
      ) {
  ...
  private _onClickOutside(e: FocusEvent) {
    ...
    if (
      relatedTarget === null ||
      (relatedTarget !== calendar && !calendar.contains(relatedTarget as HTMLElement) && relatedTarget !== this._dateTextInput)
    ) {
      this._calendar.hide();
    }

    if (e.target === this._dateTextInput && (relatedTarget === null || !(relatedTarget as HTMLElement).classList.contains("day"))) {
