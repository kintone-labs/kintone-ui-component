...
class SaturationSpectrum extends Control {
  ...
  constructor({
    ...
    height = 200,
    rgb = {r: 0, g: 0, b: 0},
    onSelect = (rgb_opt = {r: 0, g: 0, b: 0}, triggerOnChange = false) => {},
    isDisabled = false,
    isVisible = true
  ...
  fillSatSpectrumCanvas() {
    if (this.colorCanvas) {
      ...
      if (ctx) {
        ...
        const grdWhite = ctx.createLinearGradient(0, 0, this.width, 0);
        grdWhite.addColorStop(0, 'rgb(255,255,255)');
        grdWhite.addColorStop(1, 'transparent');
        ctx.fillStyle = grdWhite;
        ctx.fillRect(0, 0, this.width, this.height);
        const grdBlack = ctx.createLinearGradient(0, 0, 0, this.height);
