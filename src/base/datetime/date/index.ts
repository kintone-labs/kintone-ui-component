...
import { BASEDATE_CSS } from "./style";

let exportBaseDate;
(() => {
  exportBaseDate = window.customElements.get("kuc-base-date");
  if (exportBaseDate) {
    return;
  }

  class KucBaseDate extends KucBase {
  ...
  window.customElements.define("kuc-base-date", KucBaseDate);
  createStyleOnHeader(BASEDATE_CSS);
  exportBaseDate = KucBaseDate;
})();
const BaseDate = exportBaseDate as any;
export { BaseDate };
