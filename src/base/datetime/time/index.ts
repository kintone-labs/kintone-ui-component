...
let exportBaseTime;
(() => {
  exportBaseTime = window.customElements.get("kuc-base-time");
  if (exportBaseTime) {
    return;
  }

  class KucBaseTime extends KucBase {
    ...
    private _toggleDisabledGroup() {
      if (this.disabled)
        return this._inputGroupEl.classList.add(
    ...
    private _getPreviousMinutes(minutes: string) {
      ...
      previousMinutes =
        parseInt(previousMinutes, 10) > 5 ? "0" : previousMinutes;
  ...
  window.customElements.define("kuc-base-time", KucBaseTime);
  createStyleOnHeader(BASETIME_CSS);
  exportBaseTime = KucBaseTime;
})();
const BaseTime = exportBaseTime as any;
export { BaseTime };
