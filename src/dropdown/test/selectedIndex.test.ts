...
describe("Dropdown", () => {
  describe("selectedIndex", () => {
    ...
    it("should be changed value when updated by setter", async () => {
      ...
      expect(container.selectedIndex).to.be.equal(2);
      expect(container.value).to.be.equal(initItems[2].value);

      const selectedItemLabel = el.querySelector(
        ".kuc-dropdown__group__toggle__selected-item-label"
      );
      expect(selectedItemLabel?.textContent).to.equal(initItems[2].label);
      ...
      const svgsEl2 = itemsEl[2].querySelectorAll("svg");
      expect(svgsEl2.length).to.equal(1);
      expect(itemsEl[2].getAttribute("aria-selected")).to.equal("true");
    });
  });
});
