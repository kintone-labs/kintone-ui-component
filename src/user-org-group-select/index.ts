...
(() => {
  ...
  class KucUserOrgGroupSelect extends KucBase {
    ...
    private SMALL_ICON_SIZE = 24;
    private LARGE_ICON_SIZE = 48;
        ...
        <div class="kuc-user-org-group-select__group">
          ...
          <div class="kuc-user-org-group-select__group__container">
            ...
            >
              ...
              >
                ${this._getPickerSVGTemplateByIcon(this.icon)}
              </button>
            </div>
          </div>
          <kuc-base-error
          ...
          >
            <button
              class="kuc-user-org-group-select__group__container__select-area__selected-list__item__remove-icon__button ${
                disabled ? "kuc-user-org-group-select__group__container__select-area__selected-list__item__remove-icon__button--disabled" : ""}"
              type="button"
              ?disabled="${disabled}"
              aria-label="remove"
    ...
    private _handleClickIconButton(event: MouseEvent): void {
      event.preventDefault();
      console.log(event);
      dispatchCustomEvent(this, "click-picker-icon", event);
    }
    ...
    private _setMenuPositionAboveOrBelow() {
      ...
      if (distanceToggleButton.toBottom < distanceToggleButton.toTop) {
        // Above
        this._menuEl.style.bottom = `${this._toggleEl.offsetHeight}px`;
    ...
    private _actionHighlightFirstMenuItem() {
      ...
      for (let i = 0; i < this._matchingItems.length; i++) {
        itemIsDisabled = firstItem.classList.contains(this._DISABLED_ITEM_CLASS);
        if (itemIsDisabled) {
    ...
    private _handleClickDocument(event: MouseEvent) {
      ...
      console.log(this._disabledItemsEl);
      if (
    ...
    private _getSvgTemplateByType(
      type?: string,
      size: number = this.LARGE_ICON_SIZE,
    ) {
      if (!type) return "";
      switch (type) {
        case "user":
    ...
    private _getOrgPickerSvgTemplate() {
      return svg`
        <svg width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
          <rect x="0.5" y="0.5" width="39" height="39" fill="#3498DB" stroke="#E3E7E8"/>
